...
\fuse\ allows the filesystem to be designed in user space, which is a strong requirement by the community (since an interference with the kernel is not considered a viable option).
...
The idea behind filebundles is that we cant take advantage of the fact that the dependencies of an application are or can be known at the time of the publication of the repository, and all dependencies can be fetched simultaneously after the main (trigger) file is opened.
Filebundles operate within the existing hierarchy of \cvmfs\ (Fig.~\ref{fig-hierarchy}). They do not change the data
model; they only change \emph{when} and \emph{how} the first wave of file requests is fetched to the local cache.
